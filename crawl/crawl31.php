<?php
	...
	foreach($html->find('a') as $link)
	{
		if(strpos($link,'class="grey"') == true && strpos($link,'class="bld red fixed14"') == true)
			{
				/* echo 'Bad Link Successfully Skipped' . '<br/>'; */
				$badlink++;
				continue;
			}
		if (strpos($link->href,'www.amazon.com') == true && strpos($link->href,'amazon.com:443') == false && strpos($link,'img') == false && strpos($link,'productPromo') == false)
		{
			$newurl = $link->href;
			...
			if (strpos(implode("-", $avail),'released') == true)
			{
				...
				{
				echo 'Script Time Exceeded 25 Seconds' . '<br/>';
				break;
				}
			}
			else
	...
	echo 'Bad/Promo Skipped Found: ' . $badlink . '<br/>';
