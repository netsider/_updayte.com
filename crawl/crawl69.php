<?php
	...
	function access($host){
		$c = mysqli_connect($host, 'updayte', 'dN6UFR8GC6K6LYax');
		$database = 'queue';
...
<?php
	...
	if (isset($_POST['submit'])){
		...
		{		
			...
			$database = 'updayte';
							$con = mysqli_connect('127.0.0.1', 'updayte', 'dN6UFR8GC6K6LYax', 'updayte');
			$newurl = $link->href;
				...
				if (strpos(implode("-", $avail),'released') == true AND !strpos(implode("-", $avail),'been'))
				{
					...
					foreach($auth as $author)
					{
						for($i=0;$i <= 4;$i++)
						{
							...
							if(!empty($authorname))
							{
								...
								{
									$key_name = 'author' . (1 + $i);
									$auth_db_array = array_push_assoc($auth_db_array, $key_name, str_replace("'", '', $authorname->plaintext));
								}
							}	
						}	
					}
					...
					{ 
						...
						if (!$r4 || mysqli_num_rows($r4) == 0) 
						{
							...
							$retval = mysqli_query($con, "INSERT INTO `$current_table` (title,platform,date,source,author,author2,author3,author4) VALUES ('$db_title','$p','$db_date','$db_source','$author','$author2','$author3','$author4')");
							if(! $retval )
							{
